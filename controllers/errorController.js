...
module.exports.checkInstitutionErrors = (request) => {
    if (!request.body.institutionType) {
        throw Error('required instructorType');
    }

    if (!request.body.email) {
        throw Error('required email');
    }

    if (!request.body.address) {
        throw Error('required address');
    }

    if (!request.body.mobile) {
        throw Error('required mobile');
    }

    if (!request.body.institutionName) {
        throw Error('required institutionName');
    }

    if (!request.files['institutionLogo']) {
        throw Error('required logo');
    }
